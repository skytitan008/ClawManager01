  ...
  if (type === "ubuntu") {
    templates.push(
      ...
      },
      {
        ...
        defaultLabel: "ClawManager managed proxy path",
      },
    );
  }
  ...
  if (type === "openclaw") {
    templates.push(
      ...
      },
      {
        ...
        defaultLabel: "ClawManager generated at runtime",
      },
      {
      ...
      },
      {
        ...
        description: "Default model name injected into the OpenClaw workspace.",
        defaultValue: "auto",
      },
      {
        key: "CLAWMANAGER_LLM_PROVIDER",
        description: "Provider hint for the OpenClaw workspace.",
      ...
      {
        key: "OPENAI_API_KEY",
        description: "Compatibility alias for the gateway API key.",
        defaultLabel: "ClawManager generated at runtime",
      },
      {
        key: "OPENAI_MODEL",
        description: "Compatibility alias for the default model name.",
        defaultValue: "auto",
      },
      {
        key: "CLAWMANAGER_AGENT_ENABLED",
        description: "Toggles the ClawManager instance agent inside OpenClaw.",
        defaultValue: "true",
      },
      {
      ...
      },
      {
        ...
        description: "Disk quota injected for the instance agent.",
      ...
      },
      {
        ...
        defaultLabel: "Assigned after creation",
      },
      {
      ...
      },
      {
        ...
        description: "Protocol version expected by ClawManager agent control.",
...
const CreateInstancePage: React.FC = () => {
  ...
  const buildEnvironmentOverridesPayload = () => {
    ...
    for (const row of customEnvRows) {
      ...
      if (!hasName) {
        return { error: "Custom environment variable name is required." };
      }
      ...
      if (builtInKeys.has(name)) {
        return {
          error: `${name} is a ClawManager built-in variable. Edit it in the built-in section instead.`,
        };
      }

      if (seenCustomNames.has(name)) {
        return { error: `Duplicate environment variable name: ${name}` };
      }
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-2 text-sm text-gray-600">
                            {config.cpu_cores} CPU • {config.memory_gb} GB RAM •{" "}
                            {config.disk_gb} GB Disk
                          </div>
                        </button>
                      );
                    })}
                    ...
                    >
                      ...
                      {resourcePresetMode === CUSTOM_RESOURCE_PRESET ? (
                        ...
                        <div className="mt-3 text-sm text-gray-600">
                          Medium default: {PRESET_CONFIGS.medium.cpu_cores} CPU
                          • {PRESET_CONFIGS.medium.memory_gb} GB RAM •{" "}
                          {PRESET_CONFIGS.medium.disk_gb} GB Disk
                        </div>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="mt-5 rounded-[24px] border border-gray-200 bg-gray-50/80 p-4">
                        ...
                        >
                          {showBuiltInEnvEditor
                            ...
                            : builtinOverrideCount > 0
                              ? `Edit Built-ins (${builtinOverrideCount})`
                              : "Edit Built-ins"}
                        </button>
                      </div>
                      ...
                      {!showBuiltInEnvEditor ? (
                        <div className="mt-4 rounded-2xl border border-dashed border-gray-300 bg-white px-4 py-4 text-sm text-gray-500">
                          {builtinOverrideCount > 0
                            ? `${builtinOverrideCount} built-in override(s) configured. Expand to review or edit them.`
                            : `Using ClawManager defaults for ${builtInEnvTemplates.length} built-in variables.`}
                        </div>
                      ) : (
                        <div className="mt-4 space-y-3">
                          <div className="flex flex-wrap items-center justify-between gap-3">
                            <p className="text-sm text-gray-500">
                              Add only the built-ins you want to override.
                              Unlisted variables will keep their default
                              injected values.
                            </p>
                            <button
                              type="button"
                          ...
                          {selectedBuiltInTemplates.length === 0 ? (
                            <div className="rounded-2xl border border-dashed border-gray-300 bg-white px-4 py-4 text-sm text-gray-500">
                              No built-in overrides added yet.
                            </div>
                          ) : (
                            selectedBuiltInTemplates.map((template) => {
                              ...
                              const defaultSummary =
                                template.defaultValue !== undefined
                                  ...
                                  : `Default: ${template.defaultLabel || "ClawManager managed at runtime"}`;
                      ...
                      <div className="mt-6 border-t border-gray-200 pt-6">
                        ...
                        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                          {skillLoading ? (
                            ...
                            <div className="rounded-2xl border border-dashed border-gray-300 px-4 py-5 text-sm text-gray-500">
                              No available skills. Import skills from the
                        ...
                        {!skillLoading && availableSkills.length > 0 && (
                          <div className="mt-4 flex items-center justify-between gap-3 border-t border-gray-200 pt-4">
                            <p className="text-sm text-gray-500">
                              Page {skillPage} of {totalSkillPages} ·{" "}
                ...
                <div className="app-panel p-6">
                  ...
                  <dl className="mt-5 grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">
                        {
                          availableTypes.find((t) => t.id === formData.type)
                            ?.name
                        }
                      </dd>
                    </div>
                        ...
                        <div className="sm:col-span-2">
                          ...
                          {openClawInjectionMode === "archive" ? (
                            <p className="mt-2 text-sm text-gray-400">
                              Archive mode skips resource-based channel
                              injection.
                            </p>
                          ) : openClawPreviewLoading ? (
                            <p className="mt-2 text-sm text-gray-400">
                              Compiling OpenClaw resource preview...
                            </p>
                          ) : openClawPreviewError ? (
                            <p className="mt-2 text-sm text-red-600">
                              {openClawPreviewError}
                            ...
                            renderSummaryTagList(
                              ...
                              "No channels selected for injection.",
                              "indigo",
                            )
                          )}
                        </div>
